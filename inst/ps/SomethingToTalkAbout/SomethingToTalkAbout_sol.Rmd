...
# setwd("C:/Users/laras/OneDrive/Dokumente/Uni/BA/BA")
# ps.name = "SomethingToTalkAbout"
# sol.file = paste0(ps.name, "_sol.Rmd")
# libs = c("tidyverse", "haven", "lubridate",
#          "dotwhisker" , "stargazer",
#          "glmnet", "lfe", "skimr") # character vector of all packages you load in the problem set
...
#                     pkg.name="testversion_01",   # Name of the problem set package
#                     pkg.parent.dir = "C:/Users/laras/OneDrive/Dokumente/Uni/BA/publish/testversion_01", # Parent directory 
#                     author="Lara Santak", # Your name
#                     github.user="larasantak",     # Your github user name
...
# rps.dir = "C:/Users/laras/OneDrive/Dokumente/Uni/BA/KW19"
# # Folder in which app shall be created
# app.dir = "C:/Users/laras/OneDrive/Dokumente/Uni/BA/App/v01"
# rtutor.app.skel(ps.name=ps.name, app.name="v01",
...
One of the effects might be the weather. If the weather is harsh, for example, it is rainy. Viewership might increase due to the nature of the moviegoing inside. The weather has several attributes like temperature, precipitation, and a combination of both, like snow or hail. Adding the weather into account results in having a broad set of instruments for different weather occurrences. Some variables might explain each other so that we might have redundant variables. In total, our model with all-weather variables might be overfitted and too complicated. That means it would work well with the given data since it is perfectly molded to the data at hand. But the model might not work as good with new data. Hand-picking variables might be a good idea first, but it is hard to find the crucial variables. Therefore we will introduce a variable selection method called LASSO, which helps us to select the essential variables for our model.
...
`haven` has several functions for reading and writing statistical data formats. It includes the function `read_dta()` which reads `.dta` files up to version 15. Further information can be found [here](https://haven.tidyverse.org/reference/read_dta.html).

In order to read in data you first need to load the package `haven` with the command `library()` and use `read_dta()` accordingly:


```{r "2"}
# Example on how to use haven and read_dta():

# Load the package haven:
#library(haven)

# Read in some data "data.dta" and save it as data:
...
#< hint
cat("Load the package haven with library().")
#>

#< task
# Read in our data "data_movies.dta" and save it as data
#>
data <- read_dta("data_movies.dta")

#< hint
cat("Type data <- read_dta() and insert in the brackets data_movies.dta")
#>
...
#< hint
cat("Load the package skimr with library()")
#>

#< task
# use the function skim_without_charts() to get summary statistics of the variable production budget
#>
print(skim_without_charts(data$production_budget))

#< hint
cat("use the function skim_without_charts() to get summary statistics of the variable production budget, which can be accessed with data$production_budget. To get better readable results, use print() around skim_without_charts().")
#>
...
#< hint
cat("First load the package ggplot2 and the fill in the correct variables for each axis")
#>
#< fill_in
...
# Fill in the arguments to plot our data
plot_pb <- ggplot(data = data) +
  geom_point( aes(x = opening_sat_date, y = production_budget), na.rm = TRUE, size = 0.5 ) +
    labs(x= "date of opening Saturday", y = "production budget in dollar" )
...
#< info "dplyr: Piping (%>%), slice()"

From now on, we will use the package `dplyr`, which simplifies data manipulation. An Overview of the package can be found [here](https://dplyr.tidyverse.org). In the [Introduction](https://dplyr.tidyverse.org/articles/dplyr.html), you can find additional information about piping and functions in the package. 

Piping allows you to write a function of the form $f(x,y)$ as $x$ `%>%` $f(y)$. Whereas $x$ is the first argument entered into a function and $y$ the second argument. $x$ is often the data set or a variable. This simplifies code, especially when there are several operations applied to the same data.


```{r "2__8"}

#without pipe:
#head(data$production_budget,10)

#with pipe:
#data %>% select(production_budget) %>%
  #head(10)
...
`slice()` "slices" rows in the data by position. For further information look [here](https://dplyr.tidyverse.org/reference/slice.html).

#>

#< info "which.max(), which.min()"
The functions return the position of the maximum or minimum value. 
#>
...
#< quiz "Ex2-1"
question:  Which movie has the highest production budget?
answer: Avatar
...
#< quiz "Ex2-2"
question: How many different observations are in our data set?
answer: 24855
#>
...
#< hint
cat("Use n_distinct() on data$title")
#>
...
```
#< quiz "Ex2-3"
question: How many different movies does our data set contain?
answer: 1381
#>
...
```{r "2__12"}
#< hint
cat("Delete all arguments and press check to continue.")
#>
...
#< quiz "Ex2-4"
question: Which statement of the following is correct?
choices:
       - For each observation, we have several corresponding movies.
       - For each movie, we have one corresponding observation.
       - For each movie we have several corresponding observations.*
       - For each observation, we only have one corresponding movie.
success: Great, your answer is correct! 
failure: Are there more observations or distinct movies?
#>
...
```{r "2__13"}
#< hint
cat("Uncomment and use the function filter() with the condition from the task.")
#>
...
```{r "2__14"}
#< hint
cat("you already solved the same task for a different data set. Just copy the command")
#>
...
#< quiz "Ex2-5"
question: How many different observations are in our data set "avatar"?
answer: 18
#>
...
#< quiz "Ex2-6"
question: The gross has been calculated by...
choices:
       - summing the gross for each day in the first six weeks.
       - summing the gross for the first week.
       - summing the gross for the weekend (excluding Fridays) in the first six weeks.
       - summing the gross for the weekend (including Fridays) in the first six weeks. *
success: Great, your answer is correct! 
#>
...
gross <- gross %>% arrange(-gross)

head(gross, 10)
```
#< quiz "Ex2-7"
question: What place did Avatar score? (single digit)
answer: 2
...
#< quiz "Ex2-8"
question: Which variable do you have to select for your x-axis?
choices:
  - opening_sat_date*
  - date
  - sat_date
...
plot_gross <- ggplot(data = gross) +
  geom_point( aes(x = opening_sat_date, y = gross), na.rm = TRUE, size = 0.5)
...
#< award "Data Expert"
 Very good, you finished the exercise!
#>
...
data <- read_dta("tickets_gross.dta")
head(data)
...
library(ggplot2)
ggplot(data = data) +
  geom_point( aes( x= tickets, y = gross))
...
#< info "Code Example with lm()"
 
```{r "3__3"}
#lm( "response variable" ~ "explanatory variable", data = "data set")
```
...
my_lm <- lm( gross ~ tickets, data = data)
```
...
#< quiz "Ex3-1"
question: What does our y-intercept mean?
choices: 
- If 0 tickets are sold, the predicted gross is 15309.
- If 0 tickets are sold, the predicted negative gross is 15309.*
...
<br>
#< quiz "Ex3-2"
question: With one million more sold tickets, the gross increases by (Remember, tickets are in millions!)
answer: 6957491
#>
<br>
<br>
...
```
#< quiz "Ex3-3"
question: How much does the gross increase for a single ticket on average?
choices: 
- $5
- $6
- $7*
...
#< quiz "Ex3-4"
question:  What is the standard error in our linear regression? (The average difference between predicted gross and actual gross.)
...
#< quiz "Ex3-5"
question: To have a useful regression, do you think...
choices: 
- ... the standard error should be maximized.
- ... the standard error should be minimized.*
- ... the standard error is not essential for a regression.
...
question: Can you accept or reject the hypothesis for our regression above and why?

choices: 
  - accept (f-statistic is larger than critical value)
  - reject (f-statistic is larger than critical value)*
  - accept (f-statistic is smaller than critical value)
  - reject (f-statistic is smaller than critical value)
...
question: What does rejecting the hypothesis implicates (in general)?

choices: 
  - none of the explanatory variables have an impact
  - at least one of the explanatory variables has an impact*
  - all of the explanatory variables have an impact
...
#< award "Regression Expert"
 Well done! You finished the exercise on regression!
#>
...
#< info "Holiday variables"

In the paper are the same holiday indicators used as in Dahl and DellaVigna (2009). A short description of the holidays and naming can be found in the [Appendix A](https://www.journals.uchicago.edu/doi/full/10.1086/688177#_i42) of the paper. 
...
#< quiz "Ex4-1"
question: The y-axis (abnormal viewership) in the plot shows the difference between expected ticket sales and actual ticket sales.
choices:
- TRUE*
- FALSE
#>
...
#< quiz "Ex4-2"
question: The expected ticket sales are estimated by regression to factor out seasonal effects.
choices:
- TRUE*
- FALSE
...
#< quiz "Ex4-3"
question: What does a data point in the plot with abnormal viewership equal to zero mean?
choices:
- Ticket sales are as expected for that day. *
- Ticket sales are higher than expected for that day.
- Ticket sales are lower as expected for that day.
...
#< quiz "Ex4-4"
question: What does a data point in the plot with abnormal viewership smaller than zero mean?
choices:
- Ticket sales are as expected for that day.
- Ticket sales are higher than expected for that day.
- Ticket sales are lower as expected for that day. *
...
#< quiz "Ex4-5"
question: What does a data point in the plot with abnormal viewership larger than zero mean?
choices:
- Ticket sales are as expected for that day.
- Ticket sales are higher than expected for that day. *
- Ticket sales are lower as expected for that day.
...
#< award "Abnormal Viewership"
 Good Job! Now, you are an expert on abnormal viewership!
#>
...
choices:
-  More theaters than expected experienced a temperature between 5° and 10° Fahrenheit on this particular day.*
- Fewer theaters than expected experienced a temperature between 5° and 10° Fahrenheit on this specific day. 
- As expected, there are as many theaters that experienced a temperature between 5° and 10° Fahrenheit on this particular day.
#>
...
#< quiz "Ex5-1"
question: If there are 10% more theaters unexpectedly in the plotted range between 75°-80° Fahrenheit, the abnormal viewership of our model…
choices:
-    …increases  by 0.3 million.
-    …increases  by 3 million.
-    …decreases by 0.3 million.*
-    …decreases by 3 million.
-    …stays the same.
...
#< info "Hint"
To find out if the viewership increases or decreases with more theaters unexpectedly in the observed temperature range, you can look at the blue regression line in the plot. Does the line increase or decrease with higher x-axis values? <br>
To find the exact slope of the regression line, look at the regression output below the plot. 
#>
...
#< info "glmnet and LASSO"

`glmnet` includes not only a LASSO regression but also RIDGE and a mixture of both (elastic net). If you are interested in further information about this package, go [here](http://web.stanford.edu/~hastie/glmnet/glmnet_alpha.html#lin
). You can find further details on the exact formula used for LASSO.
...
#< quiz "Ex6-1"
question: What happens with the upper x-axis value in the plot (degree of freedom) when one coefficient of a weather shock variable is set to zero?
choices:
- The x-Axis value (degree of freedom) is increased by one.
- The x-Axis value (degree of freedom) is decreased by one.*
- The x-Axis value (degree of freedom) does not change. 
#>

<br>
<br>
#< quiz "Ex6-2"
question: How high must lambda be, that there are only two coefficients of weather shock variables not equaling zero so that we have only two degrees of freedom?
choices:
- around e^(-4)
- around e^(-3)
- around e^(-2)*
...
<br>
#< quiz "Ex6-3"
question: Which is the most important weather shock variable?
choices:
- It is the weather shock belonging to the last line in the plot that goes to zero when increasing lambda.  (The variable with the coefficient, which is set to zero last.) *
- It is the weather shock belonging to the first line in the plot that goes to zero when increasing lambda.  (The variable with the coefficient, which is set to zero first.)
#>
...
#< award "First LASSO-variable"
 Great! YOu selected the first variable!
#>
...
coef(model, s=output$lambda[3])

```
#< quiz "Ex6-4"
question: Which is the second most important weather shock variable?
choices:
- temperature_shock_90_0
- temperature_shock_50_6
- temperature_shock_70_0
- temperature_shock_75_0*
...
<!-- ToDo robust standard errors! & felm -->
...
#< quiz "Ex7-1"
question: If 10% more theaters are in the temperature range between 50° and 55° Fahrenheit, how does it influence daily viewership opening weekends? (Remember our abnormal viewership is in 1 million tickets and our weather shocks contain the percentage of theaters experiencing unexpected this wheater shock.)
choices:
  - about 400 000 less sold tickets
  - about 400 more sold ticket
  - about 400 less sold tickets
  - about 400 000 more sold ticket*
...
#< quiz "Ex7-2"
question: The f-statistic indicates in comparison to the critical value that...
choices:
  - ... we can accept the hypothesis that the variables do not influence the regressions.
  - ... we can reject the hypothesis that the variables do not influence the regressions.*
...
```{r "7__9"}
qf(.99, df1=2, df2=1669) 


```
#< quiz "Ex7-3"
question: Can we reject the hypothesis that the variables do not influence the regression?
choices:
  - yes!*
  - no!
...
fstat <- c(summary(reg_p1)$P.fstat[5], summary(reg_2)$P.fstat[5], 
           summary(reg_o1)$P.fstat[5],summary(reg_2)$P.fstat[5]) %>%
  round(digits = 3)


stargazer(reg_p1, reg_2, reg_o1, reg_2, omit = c("Constant"), type = "html",
          add.lines = list(c("f-statistic", fstat )),
          column.labels =  c("Paper", "Own Results"),
...
#< award "Finished First Stage"
 Congrats on finishing the first stage!
#>
...
#< quiz "Ex8 - 1"
question: So the increase of 100 sold tickets in the first week corresponds to...
choices:
  - ... about 40 more sold tickets overall.
  - ... about 40 more sold tickets in the second week.*
  - ... about 6.3 more sold tickets overall.
  - ... about 6.3 more sold tickets in the second week.
...
#< quiz "Ex8 - 2"
question: How big of an effect is realized in each week? 
choices:
  - Overall weeks roughly the same portion is realized each week.
  - In each week, the amount increases until it is the highest in the last week.
  - In the second week, almost half of the overall effect is realized. In the third week, one fourth and decreasing further in the following weeks. *
#>
...
#< quiz "Ex8 - 4"
question: One hundred more viewers on the weekend of the first week add up to...
choices:
  - ... about 126 more viewers in the last week.
  - ... about 126 more viewers in the first week.
  - ... about 126 more viewers in following weeks.*
  - ... about 126 more viewers in the second week.
...
question: One hundred more viewers on the weekend of the first week add up to...
choices:
  - ... about 114 more viewers in the following weeks.*
  - ... about 120 more viewers in the following weeks.
  - ... about 95 more viewers in the following weeks.
  - ... about 126 more viewers in the following weeks.
#>
<br>
<br>
...
#< quiz "Ex8 - 7"
question: Looking at the results of this exercise, which statement is true?
choices:
  - There is no relationship between abnormal viewership at the opening weekend and viewership in the following weeks.
  - Increased abnormal viewership at the opening weekend of a movie decreases the viewership in the following weeks.
  - Between abnormal viewership at the opening weekend of a movie and the following weeks is a strong link.*
#>
<br>
<br>
...
#< quiz "Ex8 - 8"
question: How much of an effect is found in subsequent weeks (compared to the opening weekend)?
choices:
  - Viewership in subsequent weeks was in total a little bit larger than at the opening weekend. That means the viewership of opening weekend more than doubled in the following weeks.*
  - Viewership in subsequent weeks was in total smaller than at the opening weekend. That means the viewership of opening weekend less than doubled in the following weeks.
  - Viewership increased for each week and duplicated three times.
#>
<br>
<br>

#< award "Finished Second Stage"
 Congratulation! This was the last exercise!
#>
...
#< quiz "Ex9 - 1"
question: Does the weather influence ticket sales?
choices:
  - Yes, we found a significant influence.*
  - No, we did not find any significant influence.
...
#< quiz "Ex9 - 2"
question: Do viewership shocks in the first week affect viewership in subsequent weeks (social spillover)?
choices:
  - No, we did not find any effect.
  - Yes, we found a multiplicate effect. *
